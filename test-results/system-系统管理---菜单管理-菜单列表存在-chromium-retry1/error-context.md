...
- Name: system.spec.ts >> 系统管理 - 菜单管理 >> 菜单列表存在
- Location: playwright/tests/system.spec.ts:104:3
...
Locator: locator('.el-table, table, [class*="table"], .el-tree').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.el-table, table, [class*="table"], .el-tree').first()
