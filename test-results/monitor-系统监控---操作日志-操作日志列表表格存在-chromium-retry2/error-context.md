...
- Name: monitor.spec.ts >> 系统监控 - 操作日志 >> 操作日志列表表格存在
- Location: playwright/tests/monitor.spec.ts:58:3
...
Locator: locator('.el-table, table').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.el-table, table').first()
