...
const itemChildren = (props.item?.children || []);
...
for (const [child] of __VLS_getVForSourceType((__VLS_ctx.itemChildren))) {
    (child.path);
    if (child.children && child.children.length > 0) {
        ...
        const __VLS_1 = __VLS_asFunctionalComponent(__VLS_0, new __VLS_0({
            index: (__VLS_ctx.resolvePath(child.path)),
        }));
        ...
        /** @type {[typeof __VLS_components.SidebarItem, ]} */ ;
        // @ts-ignore
        ...
        if (child.meta?.icon) {
            ...
            const __VLS_24 = ((child.meta.icon));
            // @ts-ignore
