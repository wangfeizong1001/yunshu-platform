...
import { httpGet, httpPost, httpPut, httpDelete, useRequest, } from '@/utils/httpClient';
...
vi.mock('axios', () => ({
    default: {
        create: vi.fn(() => ({
            request: (config) => mockRequest(config),
            get: vi.fn(),
            interceptors: {
                request: { use: vi.fn() },
                response: { use: vi.fn() },
...
describe('httpClient.ts', () => {
    ...
    describe('HTTP 便捷方法', () => {
        ...
        it('httpPost 应该发送 POST 请求', async () => {
            const result = await httpPost('/api/test', { name: 'test' });
            expect(result.success).toBe(true);
    ...
    describe('请求拦截器 Token 注入', () => {
        it('设置 Token 后请求应该被正常执行', async () => {
            ...
            const result = await httpGet('/api/auth-test');
            expect(result.success).toBe(true);
