 ...
 * @param fileName 文件名（不含扩展名）
 */
export function exportToWord(content, fileName = 'export') {
  try {
    // 创建 HTML 内容
    const htmlContent = `
      <!DOCTYPE html>
 ...
 * @param fileName 文件名（不含扩展名）
 */
export function exportToPDFFromText(content, fileName = 'export') {
  try {
    const pdf = new jsPDF();
    // 设置字体和字体大小
 ...
 * @param fileName 文件名
 */
export function exportReport(data, type, fileName = 'report') {
  switch (type) {
    case 'excel':
