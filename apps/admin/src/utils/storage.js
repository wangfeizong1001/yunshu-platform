 ...
 */
import { storageGet, storageSet, storageRemove, storageClear } from './security/storage';
/**
...
export const LocalStorage = {
     ...
     */
    set(key, value) {
        storageSet(key, value, undefined, 'local');
    },
    /**
     * 读取一个值
     * @template T 期望返回类型
    ...
    get(key, defaultValue) {
        const value = storageGet(key, 'local');
...
export const SessionStorage = {
     ...
     */
    set(key, value) {
        storageSet(key, value, undefined, 'session');
    },
    /**
     * 读取一个值（会话级别）
     * @template T 期望返回类型
     ...
     */
    get(key, defaultValue) {
        const value = storageGet(key, 'session');
        if (value === null && defaultValue !== undefined) {
            return defaultValue;
        }
        return value;
