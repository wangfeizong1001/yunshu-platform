 ...
 * 检查 ApiResponse 是否成功
 */
export function isApiResponseSuccess<T>(response: unknown): response is ApiResponse<T> {
...
export function isPaginationMeta(meta: unknown): meta is PaginationMeta {
  ...
  return (
    'page' in m && typeof m.page === 'number' &&
    'limit' in m && typeof m.limit === 'number' &&
    'total' in m && typeof m.total === 'number' &&
    'totalPages' in m && typeof m.totalPages === 'number'
  )
}
 ...
 * @returns 类型守卫函数
 */
export function createTypeGuard<T>(schema: z.ZodSchema<T>): (data: unknown) => data is T {
...
export const validateUsername = (_rule: unknown, value: string, callback: (error?: Error) => void) => {
...
export const validatePhone = (_rule: unknown, value: string, callback: (error?: Error) => void) => {
  const reg = /^1[3-9]\d{9}$/
  if (reg.test(value)) {
    callback()
  } else {
...
export const validateEmail = (_rule: unknown, value: string, callback: (error?: Error) => void) => {
  const reg = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  if (reg.test(value)) {
    callback()
  } else {
