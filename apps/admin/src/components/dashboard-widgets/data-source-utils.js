 ...
 * @returns 获取的数据结果
 */
export async function fetchDataFromSource(dataSource) {
    ...
    try {
        ...
        if (Array.isArray(rawData)) {
            ...
            if (dataSource.xField && dataSource.yField) {
                result.xData = rawData.map((item) => String(item[dataSource.xField] || ''));
                // 如果有系列字段，按系列分组
                if (dataSource.seriesField) {
                    const seriesMap = {};
                    rawData.forEach((item) => {
                        const seriesName = String(item[dataSource.seriesField] || 'default');
                        const yValue = Number(item[dataSource.yField] || 0);
                    ...
                    result.yData = rawData.map((item) => Number(item[dataSource.yField] || 0));
                }
            }
            // 饼图/环形图数据转换
            if (dataSource.nameField && dataSource.valueField) {
                result.pieData = rawData.map((item) => ({
                    name: String(item[dataSource.nameField] || ''),
                    value: Number(item[dataSource.valueField] || 0),
                }));
            }
            // 表格数据
            ...
            if (Array.isArray(dataField)) {
                ...
                // 手动处理嵌套数据
                if (dataSource.xField && dataSource.yField) {
                    result.xData = dataField.map((item) => String(item[dataSource.xField] || ''));
                    if (dataSource.seriesField) {
                        const seriesMap = {};
                        ...
                        result.yData = dataField.map((item) => Number(item[dataSource.yField] || 0));
                    }
                }
                if (dataSource.nameField && dataSource.valueField) {
                    result.pieData = dataField.map((item) => ({
                        name: String(item[dataSource.nameField] || ''),
                        value: Number(item[dataSource.valueField] || 0),
                    }));
                }
            ...
            if (dataSource.valueField) {
                result.gaugeValue = Number(rawData[dataSource.valueField] || 0);
            }
        }
        return result;
    }
 ...
 * 用于生成测试数据
 */
export function generateMockData(type) {
    switch (type) {
        case 'line':
        case 'bar':
        case 'area':
...
//# sourceMappingURL=data-source-utils.js.map