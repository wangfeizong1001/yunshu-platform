    ...
    <el-card class="table-card">
      ...
      >
        ...
        <el-table-column label="操作" width="120" align="center" fixed="right">
          <template #default="{ row }">
            <el-button link type="primary" @click="handleViewDetail(row as IOperlog)"
              >详情</el-button
            >
    ...
    <el-dialog v-model="detailVisible" title="操作日志详情" width="700px" destroy-on-close>
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="操作时间">{{
          formatDate(currentRow?.operTime)
        }}</el-descriptions-item>
        <el-descriptions-item label="操作类型">
          <el-tag :type="getOperTypeTagType(currentRow?.operType || '')">{{
            currentRow?.operType
          }}</el-tag>
        </el-descriptions-item>
        ...
        <el-descriptions-item label="操作参数" :span="2">
          <pre style="margin: 0; white-space: pre-wrap">{{
            formatJson(currentRow?.operParam)
          }}</pre>
        </el-descriptions-item>
        <el-descriptions-item label="返回结果" :span="2">
          <pre style="margin: 0; white-space: pre-wrap">{{
            formatJson(currentRow?.jsonResult)
          }}</pre>
        </el-descriptions-item>
      </el-descriptions>
    </el-dialog>
...
<script setup lang="ts">
  ...
  const handleQuery = async () => {
    ...
    try {
      ...
      const res = (await operlogApi.getOperlogPage(queryParams)) as {
        success: boolean;
        data: IOperlog[];
  ...
  const handleBatchDelete = async () => {
    try {
      await ElMessageBox.confirm(
        `确认删除选中的 ${selectedIds.value.length} 条操作日志吗？`,
        '提示',
        { type: 'warning' },
      );
