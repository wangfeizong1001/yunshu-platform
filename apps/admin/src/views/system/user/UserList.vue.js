...
/** 性别文本 */
const getSexLabel = (val) => val === USER_SEX_MALE ? '男' : val === USER_SEX_FEMALE ? '女' : '未知';
/** 用户状态 tag 类型 */
const getUserStatusTagType = (val) => val === USER_STATUS_NORMAL ? 'success' : 'danger';
/** 用户状态文本 */
const getUserStatusLabel = (val) => val === USER_STATUS_NORMAL ? '正常' : '停用';
...
__VLS_135.slots.default;
{
    ...
    const __VLS_137 = __VLS_asFunctionalComponent(__VLS_136, new __VLS_136({
        type: (__VLS_ctx.getUserStatusTagType(row.status)),
    }));
    const __VLS_138 = __VLS_137({
        type: (__VLS_ctx.getUserStatusTagType(row.status)),
    }, ...__VLS_functionalComponentArgsRest(__VLS_137));
    __VLS_139.slots.default;
    (__VLS_ctx.getUserStatusLabel(row.status));
    var __VLS_139;
}
var __VLS_135;
...
const __VLS_self = (await import('vue')).defineComponent({
    setup() {
        return {
            ...
            getUserStatusTagType: getUserStatusTagType,
