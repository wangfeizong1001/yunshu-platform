...
// 获取文件分页列表 Mock
export function getMockFilePage(params) {
    const { pageNum = 1, pageSize = 10, keyword = '', storageType = '', fileType = '' } = params;
    let filteredList = mockFileList;
    if (keyword) {
        filteredList = filteredList.filter((item) => item.fileName.includes(keyword) ||
...
// 获取文件详情 Mock
export function getMockFileDetail(fileId) {
...
// 删除文件 Mock
export function deleteMockFile(fileId) {
...
// 批量删除文件 Mock
export function batchDeleteMockFile(fileIds) {
    let deleted = false;
