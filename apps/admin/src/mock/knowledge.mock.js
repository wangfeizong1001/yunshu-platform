...
// 获取知识库文档分页列表 Mock
export function getMockKnowledgePage(params) {
    const { pageNum = 1, pageSize = 10, keyword = '', categoryId = '', status = '', visible = '' } = params;
...
// 获取知识库文档列表 Mock
export function getMockKnowledgeList(_params) {
    return mockKnowledgeList;
}
// 获取知识库文档详情 Mock
export function getMockKnowledgeDetail(knowledgeId) {
    return mockKnowledgeList.find((item) => item.knowledgeId === knowledgeId);
}
// 新增知识库文档 Mock
export function addMockKnowledge(data) {
    const newItem = {
...
// 更新知识库文档 Mock
export function updateMockKnowledge(knowledgeId, data) {
    const index = mockKnowledgeList.findIndex((u) => u.knowledgeId === knowledgeId);
    if (index !== -1) {
        mockKnowledgeList[index] = { ...mockKnowledgeList[index], ...data, updateTime: new Date().toLocaleString() };
        return mockKnowledgeList[index];
    }
    return undefined;
}
// 删除知识库文档 Mock
export function deleteMockKnowledge(knowledgeId) {
    const index = mockKnowledgeList.findIndex((u) => u.knowledgeId === knowledgeId);
    if (index !== -1) {
...
// 发布知识库文档 Mock
export function publishMockKnowledge(knowledgeId) {
    const index = mockKnowledgeList.findIndex((u) => u.knowledgeId === knowledgeId);
    if (index !== -1) {
...
// 撤回知识库文档 Mock
export function withdrawMockKnowledge(knowledgeId) {
    const index = mockKnowledgeList.findIndex((u) => u.knowledgeId === knowledgeId);
    if (index !== -1) {
...
// 新增知识库分类 Mock
export function addMockCategory(data) {
    const newItem = {
...
// 更新知识库分类 Mock
export function updateMockCategory(categoryId, data) {
    const index = mockCategoryList.findIndex((u) => u.categoryId === categoryId);
    if (index !== -1) {
        mockCategoryList[index] = { ...mockCategoryList[index], ...data, updateTime: new Date().toLocaleString() };
...
//# sourceMappingURL=knowledge.mock.js.map