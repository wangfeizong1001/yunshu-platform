...
// 短信模板 Mock 数据
export const mockSmsTemplateList = [
    {
        id: 1,
        templateCode: 'SMS_123456789',
        templateName: '登录验证码',
...
// 获取短信配置 Mock
export function getMockSmsConfig() {
    return mockSmsConfig;
}
// 获取短信模板列表 Mock
export function getMockSmsTemplatePage(params) {
    const { pageNum = 1, pageSize = 10, keyword = '', platform = '', status = '' } = params;
    let filteredList = [...mockSmsTemplateList];
    if (keyword) {
        filteredList = filteredList.filter((item) => item.templateCode.includes(keyword) ||
...
// 获取短信模板详情 Mock
export function getMockSmsTemplateDetail(id) {
    return mockSmsTemplateList.find((item) => item.id === id);
}
// 获取短信日志列表 Mock
export function getMockSmsLogPage(params) {
...
//# sourceMappingURL=sms.mock.js.map