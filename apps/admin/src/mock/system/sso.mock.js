...
// 获取 SSO 配置 Mock
export function getMockSsoConfig() {
    return mockSsoConfig;
}
// 获取 SSO 应用列表 Mock
export function getMockSsoAppPage(params) {
    const { pageNum = 1, pageSize = 10, keyword = '', appType = '', status = '' } = params;
    let filteredList = [...mockSsoAppList];
    if (keyword) {
        filteredList = filteredList.filter((item) => item.appName.includes(keyword) ||
...
// 获取 SSO 应用详情 Mock
export function getMockSsoAppDetail(id) {
    return mockSsoAppList.find((item) => item.id === id);
}
// 获取授权链接 Mock
export function getMockSsoAuthorizeUrl(appCode) {
...
// 处理 SSO 回调 Mock
export function handleMockSsoCallback(code, _state) {
    if (!code) {
        return {
            accessToken: '',
            tokenType: 'Bearer',
...
// 获取 SSO 用户信息 Mock
export function getMockSsoUserInfo(_accessToken) {
    return {
        userId: 'sso_user_001',
        username: 'sso_user',
...
// 刷新 Token Mock
export function refreshMockSsoToken(_refreshToken) {
    return {
        accessToken: `mock_access_token_${Date.now()}`,
        tokenType: 'Bearer',
        expiresIn: 7200,
...
//# sourceMappingURL=sso.mock.js.map