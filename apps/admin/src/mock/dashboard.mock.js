...
// 获取销售趋势数据 Mock
export function getMockSalesTrendData() {
    const data = [];
...
// 获取区域销售数据 Mock
export function getMockRegionSalesData() {
    return [
...
// 获取大屏看板列表 Mock
export function getMockDashboardList(params) {
    const { pageNum = 1, pageSize = 10, dashboardName = '', status = '' } = params;
    let filteredList = mockDashboardList;
    if (dashboardName) {
        filteredList = filteredList.filter((dashboard) => dashboard.dashboardName.includes(dashboardName));
    }
    if (status) {
...
//# sourceMappingURL=dashboard.mock.js.map