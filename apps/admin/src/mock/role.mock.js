...
// 获取角色分页列表 Mock
export function getMockRolePage(params) {
    const { pageNum = 1, pageSize = 10, keyword = '', status = '' } = params;
    let filteredList = mockRoleList;
    // 关键词过滤
    if (keyword) {
        filteredList = filteredList.filter((role) => role.roleName.includes(keyword) || role.roleKey.includes(keyword));
    }
    // 状态过滤
    if (status) {
...
// 获取角色详情 Mock
export function getMockRoleDetail(roleId) {
...
// 更新角色 Mock
export function updateMockRole(roleId, role) {
...
// 删除角色 Mock
export function deleteMockRole(roleId) {
    const index = mockRoleList.findIndex((r) => r.roleId === roleId);
    if (index !== -1) {
...
//# sourceMappingURL=role.mock.js.map