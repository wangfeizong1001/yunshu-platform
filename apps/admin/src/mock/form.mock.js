...
// 获取表单分页列表 Mock
export function getMockFormPage(params) {
...
// 获取表单详情 Mock
export function getMockFormDetail(formId) {
  return mockFormList.find((form) => form.formId === formId);
}
// 新增表单 Mock
export function addMockForm(form) {
  const newForm = {
...
// 更新表单 Mock
export function updateMockForm(formId, form) {
...
// 删除表单 Mock
export function deleteMockForm(formId) {
  const index = mockFormList.findIndex((f) => f.formId === formId);
  if (index !== -1) {
...
// 复制表单 Mock
export function copyMockForm(formId) {
...
//# sourceMappingURL=form.mock.js.map
