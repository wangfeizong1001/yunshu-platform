...
// 获取通知公告分页列表 Mock
export function getMockNoticePage(params) {
    const { pageNum = 1, pageSize = 10, keyword = '', noticeType = '', status = '' } = params;
    let filteredList = mockNoticeList;
    if (keyword) {
        filteredList = filteredList.filter((item) => item.noticeTitle.includes(keyword) ||
...
// 获取通知公告详情 Mock
export function getMockNoticeDetail(noticeId) {
    return mockNoticeList.find((item) => item.noticeId === noticeId);
}
// 新增通知公告 Mock
export function addMockNotice(data) {
    const newItem = {
...
// 更新通知公告 Mock
export function updateMockNotice(noticeId, data) {
...
// 删除通知公告 Mock
export function deleteMockNotice(noticeId) {
    const index = mockNoticeList.findIndex((u) => u.noticeId === noticeId);
    if (index !== -1) {
...
//# sourceMappingURL=notice.mock.js.map