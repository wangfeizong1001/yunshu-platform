...
// mock/routes/system/dept.ts
function buildDeptTree(depts, parentId = 0) {
  return depts.filter((d) => d.parentId === parentId).sort((a, b) => a.deptId - b.deptId).map((d) => ({
    ...d,
    children: buildDeptTree(depts, d.deptId),
    hasChildren: depts.some((c) => c.parentId === d.deptId)
  }));
}
function flattenDeptTree(depts) {
  const result = [];
...
var dept_default = [
   ...
   */
  {
    ...
    response: async ({ params, body }) => {
      ...
      if (body.parentId !== void 0 && body.parentId !== flatDepts[index].parentId) {
