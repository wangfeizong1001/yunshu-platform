...
function buildMenuTree(menus, parentId = 0) {
  return menus.filter((m) => m.parentId === parentId).sort((a, b) => a.orderNum - b.orderNum).map((m) => ({
    ...m,
...
var menu_default = [
  ...
  },
  {
    ...
    response: async ({ params }) => {
      ...
      const checkedKeys = db.menus.filter((m) => role.permissions.some((p) => m.perms === p)).map((m) => m.menuId);
      return success({
        menus: menuTree,
        checkedKeys
      });
   ...
   */
  {
    ...
    response: async ({ body }) => {
      ...
      const newMenu = {
        ...
        query: body.query,
        isFrame: body.isFrame !== void 0 ? body.isFrame : true,
