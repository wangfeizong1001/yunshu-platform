...
function initMessages() {
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    db.messages.push({
      ...
      readTime:
        status === '1'
          ? new Date(sendTime.getTime() + Math.random() * 2 * 60 * 60 * 1000)
              .toISOString()
              .replace('T', ' ')
              .slice(0, 19)
          : undefined,
...
function initNotifications() {
  ...
  for (let i = 1; i <= 30; i++) {
    ...
    db.notifications.push({
      ...
      pushTime:
        Math.random() > 0.2
          ? new Date(createTime.getTime() + Math.random() * 60 * 60 * 1000)
              .toISOString()
              .replace('T', ' ')
              .slice(0, 19)
          : undefined,
      expireTime: new Date(createTime.getTime() + 30 * 24 * 60 * 60 * 1000)
        .toISOString()
        .replace('T', ' ')
        .slice(0, 19),
      createBy: 'admin',
        ...
        : i % 20 === 0
          ...
          : i % 30 === 0
            ...
            : i % 40 === 0
              ...
              : '登录';
    const status =
      operation === '登出' || operation === '注册' ? '0' : Math.random() > 0.1 ? '0' : '1';
    const msg =
      status === '0'
        ? `${operation}成功`
        : operation === '登录'
          ? `${operation}失败：${errorMsgs[Math.floor(Math.random() * errorMsgs.length)]}`
          : `${operation}失败`;
...
function initOnlineUsers() {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    db.onlineUsers.push({
      ...
      lastAccessTime: new Date(Date.now() - Math.random() * 30 * 60 * 1000)
        .toISOString()
        .replace('T', ' ')
        .slice(0, 19),
      expireTime: new Date(loginTime.getTime() + 30 * 60 * 1000)
        .toISOString()
        .replace('T', ' ')
        .slice(0, 19),
      userId: String((i % 8) + 1),
...
function initJobLogs() {
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    db.jobLogs.push({
      ...
      error:
        status === '1'
          ? ['NullPointerException', 'Connection timeout', 'SQLException', 'BusinessException'][
