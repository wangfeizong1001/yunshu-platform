...
function formatUser(user: User) {
  return {
    ...
    roles: db.roles.filter(r => user.roleId.includes(r.roleId)).map(r => ({ roleId: r.roleId, roleName: r.roleName, roleKey: r.roleKey }))
  }
}
...
export default [
   ...
   */
  {
    ...
    method: 'get',
    response: async ({ query }: { query: { pageNum?: string; pageSize?: string; keyword?: string; status?: string; deptId?: string } }) => {
      await randomDelay()
   ...
   */
  {
    ...
    method: 'get',
    response: async ({ query }: { query: { keyword?: string; status?: string; deptId?: string } }) => {
      await delay()
   ...
   */
  {
    ...
    response: async ({ params, body }: { params: { userId: string }; body: any }) => {
      ...
      // 检查用户名是否与其他用户冲突
      if (body.username && db.users.some(u => u.username === body.username && u.userId !== parseInt(params.userId))) {
        return fail('用户名已存在')
      }
   ...
   */
  {
    ...
    method: 'put',
    response: async ({ params, body }: { params: { userId: string }; body: { status: string } }) => {
      await delay()
   ...
   */
  {
    ...
    method: 'put',
    response: async ({ params, body }: { params: { userId: string }; body: { password: string } }) => {
      await delay()
   ...
   */
  {
    url: '/api/system/user/:userId/posts',
    method: 'put',
    response: async ({ params, body }: { params: { userId: string }; body: { postIds: number[] } }) => {
      await delay()
   ...
   */
  {
    url: '/api/system/user/:userId/roles',
    method: 'put',
    response: async ({ params, body }: { params: { userId: string }; body: { roleIds: number[] } }) => {
      await delay()
