...
### 🟤 第7批 — 测试覆盖补充
**目标**：建立视图层测试基线，覆盖核心缺失模块的 E2E。（👉 本批次与 [测试覆盖报告](file:///workspace/docs/test-coverage-report.md) 联动执行）
**预计总工时**：2 天
**前置依赖**：第1-6批功能全部完成（测试需基于稳定功能）
**状态**：⏳ 进行中（7.4/7.6/7.7 已部分覆盖，需强化；7.1/7.2/7.3/7.5 需新建）

| # | 任务标题 | 涉及文件 | 任务详情 | 验收标准 | 工时 | 状态 | 关联报告项 |
|---|---------|---------|---------|---------|------|------|-----------|
| 7.1 | 公共组件单元测试 | [CommonTable.test.ts](file:///workspace/apps/admin/src/__tests__/components/CommonTable.test.ts)（新建）, [LanguageSwitch.test.ts](file:///workspace/apps/admin/src/__tests__/components/LanguageSwitch.test.ts)（新建）, [TenantSelect.test.ts](file:///workspace/apps/admin/src/__tests__/components/TenantSelect.test.ts)（新建） | 使用 `@vue/test-utils` + `vitest`；对 CommonTable 测试 props data/columns/pagination、emit 事件；对 LanguageSwitch 测试中英文切换触发 setLocale；对 TenantSelect 测试租户切换后调用 setTenantId | 新增 3+ 测试文件；`pnpm --filter @yunshu/admin test` 全部通过 | 3h | ⬜ | [test-coverage-report.md §3.1 缺失](file:///workspace/docs/test-coverage-report.md#_31-appsadmin-前端单元测试) |
| 7.2 | Store 状态管理测试 | [store/app.test.ts](file:///workspace/apps/admin/src/__tests__/store/app.test.ts)（新建）, [store/user.test.ts](file:///workspace/apps/admin/src/__tests__/store/user.test.ts)（新建）, [store/permission.test.ts](file:///workspace/apps/admin/src/__tests__/store/permission.test.ts)（新建）, [store/tagsView.test.ts](file:///workspace/apps/admin/src/__tests__/store/tagsView.test.ts)（新建） | 对每个 store 测试初始值、actions/mutations 执行后的状态变化；mock API 层避免真实请求；测试 localStorage 持久化（使用 vitest mock） | 每个 store 至少 5+ 用例，覆盖核心 action | 4h | ⬜ | [test-coverage-report.md §3.1 缺失](file:///workspace/docs/test-coverage-report.md#_31-appsadmin-前端单元测试) |
| 7.3 | 路由守卫测试 | [router/guard.test.ts](file:///workspace/apps/admin/src/__tests__/router/guard.test.ts)（新建） | 创建测试用 router 实例 + mock userStore + mock permissionStore；测试：未登录访问受保护页应跳登录；已登录访问 `/login` 应跳首页；无 userId 时触发 getUserInfo + generateRoutes；登出后 isDynamicRouteAdded 被重置 | 关键路径 6+ 用例均通过 | 3h | ⬜ | [test-coverage-report.md §3.1 缺失](file:///workspace/docs/test-coverage-report.md#_31-appsadmin-前端单元测试) |
| 7.4 | 工作流 E2E 增强 | [playwright/tests/workflow.spec.ts](file:///workspace/playwright/tests/workflow.spec.ts)（已存在） | 现有文件仅覆盖基础列表/设计器，**需补充**：① 发布流程后状态断言；② 挂起后状态断言；③ 激活后状态断言；④ 删除流程二次确认弹窗断言；⑤ 任务审批通过/驳回流程；⑥ 转办/加签/委托操作 | `pnpm test:e2e` 通过；新增 6+ 场景；流程操作完整闭环 | 3h（基于现有基础上增强） | ⏳ | [test-coverage-report.md §3.2 部分覆盖](file:///workspace/docs/test-coverage-report.md#_32-playwright-e2e) |
| 7.5 | system 缺失子模块 E2E 新建 | [system-config.spec.ts](file:///workspace/playwright/tests/system-config.spec.ts)（新建）, [system-file.spec.ts](file:///workspace/playwright/tests/system-file.spec.ts)（新建）, [system-notice.spec.ts](file:///workspace/playwright/tests/system-notice.spec.ts)（新建）, [system-message.spec.ts](file:///workspace/playwright/tests/system-message.spec.ts)（新建）, [system-knowledge.spec.ts](file:///workspace/playwright/tests/system-knowledge.spec.ts)（新建）, [system-oss.spec.ts](file:///workspace/playwright/tests/system-oss.spec.ts)（新建）, [system-sms.spec.ts](file:///workspace/playwright/tests/system-sms.spec.ts)（新建）, [system-sso.spec.ts](file:///workspace/playwright/tests/system-sso.spec.ts)（新建）, [system-third.spec.ts](file:///workspace/playwright/tests/system-third.spec.ts)（新建） | 共 9 个 spec；每个测试：① 页面能打开；② 表格有数据；③ 搜索可筛选；④ 新增/编辑对话框可打开；⑤ 删除二次确认 | 新增 9 个 spec 文件；基础场景覆盖 | 4.5h | ⬜ | [test-coverage-report.md §3.2 缺失](file:///workspace/docs/test-coverage-report.md#_32-playwright-e2e) |
| 7.6 | 个人中心 E2E 增强 | [user.spec.ts](file:///workspace/playwright/tests/user.spec.ts)（已存在） | 现有文件覆盖基础加载/输入，**需补充**：① 修改密码-旧密码错误场景（断言"原密码错误"提示）；② 修改密码-成功场景（断言"修改成功"+ 重新登录验证）；③ 头像上传预览；④ 基本信息保存成功场景 | 所有步骤有断言；修改密码含正反用例 | 2h（基于现有基础上增强） | ⏳ | [test-coverage-report.md §3.2 部分覆盖](file:///workspace/docs/test-coverage-report.md#_32-playwright-e2e) |
| 7.7 | 布局组件 E2E 增强 | [layout.spec.ts](file:///workspace/playwright/tests/layout.spec.ts)（已存在） | 现有文件覆盖基础导航/侧边栏，**需补充**：① TagsView 右键菜单弹出；② 关闭当前/其他/全部三种操作；③ 全局搜索框输入关键字；④ 搜索结果跳转到对应页面 | 布局交互全部断言验证 | 2h（基于现有基础上增强） | ⏳ | [test-coverage-report.md §3.2 部分覆盖](file:///workspace/docs/test-coverage-report.md#_32-playwright-e2e) |
...
**完成说明**：
- ✅ 品牌色统一为 `#4a9eff`，全局通过 `var(--el-color-primary)` 引用
- ✅ 创建 `.stylelintrc.js`，添加 `color-no-hex` 规则
- ✅ ESLint 配置 `no-restricted-syntax` 规则检测 JS/TS 中的硬编码颜色
- ✅ CI 集成 `lint-style` job
- ✅ 修复 300+ 处硬编码颜色，布局文件零硬编码
- ✅ `#409eff`（Element Plus 默认主色）全部替换完成
- ✅ 保留专用色板：大屏可视化色、流程图节点渐变、代码高亮主题
