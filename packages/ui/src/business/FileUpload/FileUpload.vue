...
async function startUpload() {
  ...
  for (const file of fileList.value) {
    ...
    uploadProgress.value = Math.min(100, uploadProgress.value + Math.round(100 / fileList.value.length));
  }
  ...
  <div class="yun-upload">
    ...
    <div v-if="fileList.length > 0" class="yun-upload__list">
      <div v-for="(file, idx) in fileList" :key="idx" class="yun-upload__file">
        <img v-if="isImage && previewUrls[idx]" :src="previewUrls[idx]" class="yun-upload__preview">
      ...
      <div class="yun-upload__actions">
        ...
        <button class="yun-upload__btn yun-upload__btn--clear" @click="clearFiles">
          清除
        </button>
      </div>
...
.yun-upload__area:hover { border-color: var(--primary); background: var(--primary-alpha-10); }
.yun-upload__area.is-dragging { border-color: var(--primary); background: var(--primary-alpha-20); }
.yun-upload__area.is-disabled { opacity: 0.5; cursor: not-allowed; }

.yun-upload__icon { font-size: 40px; margin-bottom: 12px; }
.yun-upload__text { color: var(--text-secondary); margin: 0 0 8px; font-size: var(--font-size-base); }
.yun-upload__hint { color: var(--text-muted); font-size: var(--font-size-sm); margin: 0; }

.yun-upload__list { margin-top: 16px; }
.yun-upload__file {
  display: flex;
  align-items: center;
  gap: 12px;
...
.yun-upload__preview { width: 40px; height: 40px; object-fit: cover; border-radius: var(--radius-sm); }
.yun-upload__filename { flex: 1; font-size: var(--font-size-sm); color: var(--text-primary); }
.yun-upload__filesize { font-size: var(--font-size-xs); color: var(--text-muted); }

.yun-upload__actions { display: flex; gap: 8px; margin-top: 12px; }
.yun-upload__btn {
  padding: 8px 20px;
...
.yun-upload__btn:disabled { opacity: 0.5; cursor: not-allowed; }
